...
async def restart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id_str = str(user_id) # Сразу приведем к строке
    
    # --- 1. Проверка/Создание ключа ---
    ensure_global_user_secret(user_id_str)
    # ----------------------------------
    ...
    # Фоллбэк: если ключа нет (чего быть не должно из-за ensure_global_user_secret), создаем
    if not secret_key:
        secret_key = generate_secret_key()
        secret_ref.set(secret_key)

    # Добавляем параметр secret в ссылку
    webapp_url = f"https://novel-qg4c.onrender.com/{user_id_str}?secret={secret_key}"
    # -------------------------------------

    keyboard = [
        [InlineKeyboardButton("🌠Создать историю", callback_data='create_story_start')],
        # Кнопка теперь содержит защищенную ссылку
        [InlineKeyboardButton("🦊Создать/редактировать через web", url=webapp_url)],
        [InlineKeyboardButton("📂 Загрузить прохождение", callback_data='load_menu_start')], 
        [InlineKeyboardButton("✧ 〰️〰️✦〰️〰️ ✧", callback_data='ignore')],                
        [InlineKeyboardButton("✏️Посмотреть мои истории", callback_data='view_stories')],
    ...
    message_text = (
        ...
        'При выборе создания истории через web откроется удобный функциональный вебредактор. '
        'Любую историю в любой момент вы можете открывать или редактировать как через него так и внутри бота.'
    )
...
async def neural_story(update: Update, context: ContextTypes.DEFAULT_TYPE, clean_title: str) -> int:
    ...
    async def background_generation():
        ...
        try:
            ...
            try:
                await waiting_message.edit_text(
                    "⚠️ Произошла ошибка при генерации истории.  Нейросеть может присылать неверные данные из-за того что задача довольно сложна для неё. Попробуйте ещё раз позже."
                )
            except Exception as e_edit:
...
async def clean_html_for_telegram_final(html_text: str) -> str:
    """
    Асинхронно очищает HTML-текст от любых тегов, кроме разрешённых Telegram.
    """
    allowed_tags = {
        "b", "strong", "i", "em", "u", "ins",
        "s", "strike", "del", "code", "pre", "a", "span",
    ...
    for tag in soup.find_all(True):  # True -> находит все теги
        if tag.name not in allowed_tags:
            tag.unwrap()  # удалить тег, но сохранить его содержимое
...
async def render_fragment(
    ...
    if choices_data and visible_button_count == 0 and not auto_transition_target_fragment_id:
        if not neuro_mode: # Предполагаем, что в нейро-режиме тупиков быть не должно, или они обрабатываются иначе
            ...
            clean_current_display_text_parts.append(dead_end_suffix)

    final_base_text_for_display = "".join(clean_current_display_text_parts)
...
async def generate_neural_story_full(query):
    ...
    system_instruction = (
        ...
        "       - \"5\": установить точное значение, сбросив предыдущие.\n"
        "       - \">5\", \"<3\", \"=1\": ПРОВЕРКА. Если условие не выполнено, кнопка недоступна.\n"
        "   - \"hide\": true/false. Если true — игрок не видит уведомления о получении стата или кнопки проверки которых он не прошёл.\n"
        "2. ОТОБРАЖЕНИЕ АТРИБУТОВ:\n"
        "   В поле \"text\" используй {{stat_name}} для подстановки значений. Пример: \"У вас {{gold}} монет.\"\n"
        "   Важно! {{stat_name}} в начале истории не существует. Их нужно именно задать в effects, убедиться что на момент прохождения фрагмента у пользователя уже есть этот stat_name и только потом выводить это значение в сообщении, если нужно .\"\n"        
        "   Важно! Иными словами нет никакого смысла выводить {{stat_name}} в первых фрагментах. У пользователя просто нет ещё никаких характеристик, они ещё не заданы \"\n"        
        ...
        "   Если в поле \"text\" внутри choices указано число в виде строки (например, \"5\"), то кнопка не отображается, а бот сам переключит на target через 5 секунд.\n\n"
        ...
        "- Предпочтение давай более простым с точки зрения проверок и эффектов сюжетам, чтобы самой в них не запутаться.\n"        
    ...
    context = (
        ...
        "Начни строго с 'main_1'. Следи за логикой атрибутов и используй 'effects' и проверки, чтобы усилить погружение."
    )
    try:
        google_search_tool = Tool(google_search=GoogleSearch())
        response = await client.aio.models.generate_content(
